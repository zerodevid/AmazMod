...
Taken from a Verge mid-trip. The disc holds the compass here because Maps was naming a direction
rather than a turn - the needle points east, which is what the line below it says. The bar is the
route in Maps' own traffic colours. There is no distance to the next manoeuvre because Maps does not
publish one until the phone is actually moving.
