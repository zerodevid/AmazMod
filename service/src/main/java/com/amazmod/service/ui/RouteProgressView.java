...
public class RouteProgressView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < lengths.length && i < colours.length; i++) {
            final float span = width * Math.max(0, lengths[i]) / (float) total;
            if (span <= 0)
                continue;

            final int colour = colours[i];
            // Wholly behind the marker, so dim the whole stretch
            final boolean behind = (travelledX >= 0) && (x + span <= travelledX);

            paint.setColor(colour);
            paint.setAlpha(behind ? TRAVELLED_ALPHA : 255);

            rect.set(x, 0, x + span, height);
            canvas.drawRoundRect(rect, cornerRadius, cornerRadius, paint);

            // The stretch the marker sits inside gets dimmed only up to the marker, so the boundary
            ...
            if (travelledX > x && travelledX < x + span) {
                ...
                rect.set(x, 0, travelledX, height);
                canvas.drawRoundRect(rect, cornerRadius, cornerRadius, paint);
            }
